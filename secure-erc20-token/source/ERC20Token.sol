...
/** 
 * @notic For new ERC20 token deployments:
...
interface IERC20 {
    ...
    ///Transfers _tokens amount of tokens to address _to, and fires the Transfer event.
    function transfer(address _to, uint256 _tokens) external returns (bool);
    
    ///Allows _spender to withdraw from your account multiple times, up to the _tokens amount.
    function approve(address _spender, uint256 _tokens) external returns (bool);
    
    ///Transfers _tokens amount of tokens from address _from to address _to, and fires the Transfer event
    function transferFrom(address _from, address _to, uint256 _tokens) external returns (bool);
    
    ///Returns the amount which _spender is still allowed to withdraw from _account
 ...
 * @author https://github.com/OpenZeppelin/openzeppelin-contracts
...
/**
 * @title Secure ERC20 Token
 * @author Madiba Research Group
 * @notice You can use this contract for only educational purpose
 * @dev Comaptibel with solidity 0.5.11 and above
 */
contract ERC20 is IERC20{
    using SafeMath for uint256;		/// Using SafeMath to mitigate integer overflow

    string public name = "Ethereum Secure Token";  /// Name of the token
    string public symbol = "EST";                  /// Token symbol
    
    uint8 public exchangeRate = 100;                        /// 1 ETH = 100 tokens
    uint256 public decimals = 18;                           /// Divisible to 18 decimal places
    uint256 private coefficient = 10 ** decimals;           /// Conversion coefficient

    mapping(address => mapping (address => uint256)) private allowances;	/// Allowed token to transfer by spender
    ...
    uint256 private initialSupply = 100000000;              /// Controls economy of the token
    ...
    event LogRateChange(uint8 _current, uint8 _new);
    ...
    constructor(uint256 _totalSupply) public {
        ...
        initialSupply = (_totalSupply == 0) ? initialSupply : _totalSupply;
        balances[owner] = initialSupply;                                    /// Assigning initial tokens to the owner
        emit Transfer(address(0), owner, initialSupply);                    /// Logs transfered tokens to the owner
    }
    ...
    ///Returns the account balance of another account with address _tokenHolder
    ...
    /**
     * @dev Special type of Transfer and make it possible to give permission to another address 
    ...
    function transferFrom(address _from, address _to, uint256 _tokens) public returns (bool success) {
        ...
        require(_tokens <= (                                /// Mitigates multiple Withdrawal Attack
                            (allowances[_from][msg.sender] > transferred[_from][msg.sender]) ? 
                             allowances[_from][msg.sender].sub(transferred[_from][msg.sender]) : 0)
    ...
    /**
     * @dev It helps to give permission to another address to spend tokens on your behalf.
    ...
    function approve(address _spender, uint256 _tokens) public returns (bool success) {
        ...
        allowances[msg.sender][_spender] = _tokens;     /// Sets allowance of the spender
    ...
    function allowance(address _tokenHolder, address _spender) public view returns (uint256 tokens) {
        return allowances[_tokenHolder][_spender];     /// returns allowance of _spender
    }
    
    /**
     * @dev Transferring ETH to the seller of the token
    ...
    function sell(uint256 _tokens) public returns(bool success)
    {
        ...
        uint256 _wei = _tokens.div(exchangeRate).mul(coefficient);      /// Token to ETH and then ETH to Wei
    ...
    /**
     * @dev Withdraw ETH from contract and send it to the address that is specified by the owner
     * Only owner of the contract can call this function
     */
    function withdraw(uint256 _eth) public onlyOwner returns(bool success){
        uint256 _wei = _eth.mul(coefficient);           /// ETH to Wei
        require(address(this).balance >= _wei);         /// Contract has enough ETH?
        msg.sender.transfer(_wei);                        /// Transferring Wei to _to
        emit LogWithdrawal(owner, address(this), _wei, msg.sender);  
        return true;                /// Owner requested the Contract to send _wei to _to
    }
    
    /// Supports buying token by transferring ETH
    function buy() public payable returns(bool success){
        require(msg.sender != owner);                   /// Token owner cannot be as seller and buyer
        uint256 _eth = msg.value.div(coefficient);      /// Wei to ETH
    ...
    function contractBalance() public view onlyOwner returns(uint256 eth){
        return address(this).balance.div(coefficient);  /// Wei to ETH
    }
    
    /// Sets new exchange rate
    function setExchangeRate(uint8 _newRate) public onlyOwner returns(bool success)
    {
        uint8 _currentRate = exchangeRate;
        exchangeRate = _newRate;
        emit LogRateChange(_currentRate, _newRate);
        return true;
    }
