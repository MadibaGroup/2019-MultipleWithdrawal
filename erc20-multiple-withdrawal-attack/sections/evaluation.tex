...
Several solutions have been proposed by Ethereum community---mostly from developers on GitHub\cite{Ref07}---to address the attack. There would be some trad-offs for each solution that needs to be evaluated in term of conforming with constraints of the standard and attack mitigation. We have examined each solution and explained possible ERC20 constraint violation.

\subsection{Enforcement by User Interface (UI)}\label{sec:enfui}

% Jeremy: We must define UI more specifically

ERC20 standard recommends to (1) set allowance to zero before any non-zero values and (2) enforce approval processing check in UI instead of smart contract (see screenshot~\ref{fig:uie}). This approach can not prevent the attack in the below scenario \cite{Ref14}:
